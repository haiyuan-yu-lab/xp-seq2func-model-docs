...
This site documents release **0.1.0a3**.
...
| `EncoderPredictor` | Model | Top-level model: encoder (`ConvEncoder` or `ConvSelfAttEncoder`) + one or more `ClassPredictor` heads |

!!! warning "Alpha release"
    **0.1.0a3** is a pre-release. Config schemas and CLI flags may change. Pin
    the tag if you need a fixed cut.

## Where to start

1. [Install](install.md) from the `v0.1.0a3` code tag
